...
- `RequestHandler.Middleware` exposes registration metadata only (`MiddlewareDescriptor`: the middleware type for `Use<T>()`, null type + method name for delegates) — never the component delegates or the compiled pipeline. List order is registration order, which is inbound execution order.
