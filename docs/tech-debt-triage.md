...
- **#009** — Missing test coverage for critical paths → coverage closed across batches A/B/C plus follow-on test passes. Suite now at 40 tests, 99%+ line coverage on `Plumber`. Concurrent-invocation tests intentionally skipped: pipeline build is delegated to `Lazy<T>` (BCL contract), per-invocation state is scoped, threaded tests would add CI flakiness without protecting an invariant we own. The stale `Create(args, configure)` bullet is N/A — that overload no longer exists.
