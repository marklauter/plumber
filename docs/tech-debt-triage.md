...
**Totals:** 19 entries — 11 Resolved, 8 Open.
...
### Cross-cutting
- **#009** — Missing test coverage for critical paths (timeout, disposal, cancellation, error cases, concurrency, configuration loading, the multi-Build "recipe" semantics, etc.). **Don't tackle as a standalone PR.** Fold tests into batches A, B, C — every fix should arrive with a regression test.
